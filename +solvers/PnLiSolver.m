classdef PnLiSolver < handle
    %% PnLiSolver - Projected Newton optimization algorithm on custom set
    ...
    methods (Access = public)
        ...
        function self = PnLiSolver(nlp, varargin)
            %% Constructor
        ...
        function self = solve(self)
            ...
            while self.iStop == 0
                ...
                % Compute descent direction { H*d = -g | B*C*d = 0 }
                d = self.nlp.minEqProject(g, H, ~working);
    ...
    methods (Access = private)
        ...
        function printHeaderFooter(self, msg)
            switch msg
                case 'header'
                    ...
                    self.printf('\n');
                    self.printf('%s\n', ['*', repmat('-',1,58), '*']);
                    self.printf([repmat('\t', 1, 3), 'PnLi Solver \n']);
                    self.printf('%s\n\n', ['*', repmat('-',1,58), '*']);
        ...
        function working = getWorkingSet(self, x, g, H)
            ...
            gFixed = (Cx - self.nlp.cL < appZero & Cd < 0) | ...
            ...
            fixed(~gFixed) = (Cx - self.nlp.cL(~gFixed) < appZero ...
                & Cd < 0) | (self.nlp.cU(~gFixed) - Cx < appZero & Cd > 0);
